...
### Выходные структуры функции process_points

Функция `process_points(bin_image)` возвращает три структуры данных:

1. **res** — словарь парных групп точек по уровням (строкам):
   - Тип: `dict[int, list[list[tuple[float, float]]]]`
   - Ключ: номер уровня (строки)
   - Значение: список из двух групп: `[правая_группа, левая_группа]`
   - Каждая группа — это список точек [(x, y)]
   - Пример:
     ```python
     res = {
         1: [right_group_1, left_group_1],
         2: [right_group_2, left_group_2],
         ...
     }
     # где right_group_1 = [(x1, y1), (x2, y2), ...]
     ```
   - Используется для анализа строк, вычисления наклона, полей, визуализации парных групп.

2. **(min_x, max_x)** — границы облака точек по оси X:
   - Тип: `tuple[float, float]`
   - Описание: минимальное и максимальное значение x среди всех нормализованных точек.
   - Используется для расчёта полей и нормализации.

3. **sort** — словарь вертикальных групп точек по x:
   - Тип: `dict[float, list[list[tuple[float, float]]]]`
   - Ключ: x-координата (float)
   - Значение: список групп (каждая — список точек с этим x)
   - Пример:
     ```python
     sort = {
         0.12: [
             [(0.12, 0.5), (0.12, 0.51), ...],  # группа 1 по x=0.12
             [(0.12, 0.7), (0.12, 0.71), ...],  # группа 2 по x=0.12
         ],
         0.15: [
             [(0.15, 0.3), (0.15, 0.31), ...],
         ],
     ...
     ```
   - Используется для анализа вертикальных структур, поиска столбцов, построения итоговых групп по строкам.

**Все координаты точек нормализованы в диапазон [-1, 1].**
